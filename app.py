...
class VannaFlaskAPI:
    ...
    def __init__(
        ...
        self.flask_app.secret_key = os.getenv('SECRET_KEY')

        self.swagger = Swagger(
          self.flask_app, template={"info": {"title": "Chitti GPT"}}
        )
    ...
    def run(self, *args, **kwargs):
        ...
        if args or kwargs:
            ...
            try:
                ...
                output.serve_kernel_port_as_window(8089)
                from google.colab.output import eval_js

                print("Your app is running at:")
                print(eval_js("google.colab.kernel.proxyPort(8089)"))
            except:
                print("Your app is running at:")
                print("http://localhost:8089")

            self.flask_app.run(host="0.0.0.0", port=8089, debug=self.debug, use_reloader=False)
...
class VannaFlaskApp(VannaFlaskAPI):
    def __init__(
        ...
        logo="static/chittiface.2e6bc68f5475c3761824.png",
        ...
        def proxy_vanna_svg():
            remote_url = "http://172.232.71.153/static/media/chittiface.2e6bc68f5475c3761824.png"
...
temp_ = VannaFlaskApp(
    vn=vn,
    auth=SimplePassword(users=[{"email": "sadakpramodh_maduru@welspun.com", "password": "password"}]),
    allow_llm_to_see_data=True,
    show_training_data=True,
    sql=True,
    table=True,
...
#         print("Loaded object from pickle file")
#         loaded_object.run()
        
# except (FileNotFoundError, pickle.UnpicklingError, EOFError) as e:
#     print(f"Error loading pickle file: {e}")
#     with open(file_path, 'wb') as file:
#         pickle.dump(temp_, file)
temp_.run()
